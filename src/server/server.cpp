...
namespace robot {
  ...
  auto update_state(aris::server::ControlServer& cs) -> void
  {
    ...
    //��ȡ�����������ݣ��������˲�--������������������
    //����ֻ�Ǽ�ͨ����վ��������6�����жϣ����߸���վ������ioҲ�����������������û���Ҫͨ��FS_NUM���趨
  ...
  auto get_state_code() -> std::int32_t
    //��ȡ״̬�֡���100:ȥʹ��,200:�ֶ�,300:�Զ�,400:����������,410:������ͣ��,420:����ֹͣ��500:����
  {
  ...
  auto createControllerROSMotorTest() -> std::unique_ptr<aris::control::Controller>
  {
    ...
    for (aris::Size i = 0; i < 12; ++i)
    {
      ...
      double pos_offset[12]
      {
        ...
        // -1.4974336956172, 0.128106570548551, 0.844257485597249,
        // -1.4974336956172, 0.128106570548551, 0.844257485597249,
        // -1.4974336956172, 0.128106570548551, 0.844257485597249, // �ȣ� 85�� 7.4, 48.35
...
struct MotorConfig::Imp {
    ...
    double feed_constant_ = 360; //当量比(机械结构行程/减速机输出端转1圈)--直线轴:导程mm/1圈;旋转轴:360度/1圈
    double gear_ratio_ = 1.0;    //减速比
    int resolution_ = 131072;       //电机分辨率
    int direction_ = 1;        //1：电机顺时针方向；-1：电机逆时针方向------
    double unit_factor_ = 180.0 / aris::PI;
    std::string  motion_type_ = "rotatory"; // 1表示旋转 ； 2表示直线
    uint32_t current;
    uint32_t torque;
